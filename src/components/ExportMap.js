...
const ExportMap = ({year, zoom, activeCountryData, updateActiveCountry}) => {
  ...
  const handleCountryHover = async (alpha2, name) => {
    try {
      const arms_export_response = await fetch(`https://${HOST}:${API_PORT}/arms/exports/total?country_code=${alpha2}&year=${year}`); 
      const arms_export_data = await arms_export_response.json();
      
      const merch_export_response = await fetch(`https://${HOST}:${API_PORT}/merchandise/exports/total?country_code=${alpha2}&year=${year}`)
